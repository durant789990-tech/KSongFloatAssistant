...
public class PermissionWizardActivity extends Activity {
    ...
    private boolean ok() {
        switch (step) {
            ...
            case 4: return false;
            default: return com.zzy.ksongfloat.security.SecureStorage.hasApiKey(this) && com.zzy.ksongfloat.ai.AiSettingsRepository.load(this).isUsable();
        }
    }

    private String statusText() {
        if (step == 4) return "当前不可检测，请按说明人工确认";
        return ok() ? "已开启" : "未开启";
    }
    ...
    private void openSetting() {
        switch (step) {
            ...
            case 2: if (Build.VERSION.SDK_INT >= 33) requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS}, 3301); else SettingsNavigator.openNotification(this); break;
            case 3: SettingsNavigator.openBattery(this); break;
            case 4: XiaomiPermissionHelper.openAutoStart(this); break;
            default: AppNavigator.aiSettings(this); break;
        }
    }

    private void next() {
        if (step < 5) { step++; render(); }
        else { getSharedPreferences("first_run", MODE_PRIVATE).edit().putBoolean("wizard_seen", true).apply(); finish(); }
    }
}
